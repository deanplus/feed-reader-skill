...
`sync` reports each source to stderr as it starts. Use `--source <id>` to sync only one registered source. Use `--no-items` with JSON output to return only the new-item count and per-source results; call `items` when content is needed.
